...
# Then run this code:
get_data <- function(url, res, filenames) {
  unzip("HydroRIVERS_v10_as_shp.zip") #unzip, need once only
  filenames <- list.files("HydroRIVERS_v10_as_shp", pattern="*.shp", full.names=T)
  return(filenames)
}
...
get_bounding_box <- function(crsLONGLAT, bbox, new_prj, bb) {
  ...
  #  c(35.000, 35.000, 69.5, 69.5, 35.000)     # y-coordinates (latitudes) of points A,B,C,D
    c(60.000, 100.000, 100.000, 60.000, 60.000), # x-coordinates (longitudes) of points A,B,C,D
    c(5.000, 5.000, 40.000, 40.000, 5.000)     # y-coordinates (latitudes) of points A,B,C,D
  #  c(60, 70,70,60,60), # x-coordinates (longitudes) of points A,B,C,D
  #  c(20,20,35,35,20)     # y-coordinates (latitudes) of points A,B,C,D
    ))),
  crs = crsLONGLAT)
...
p1 <- get_river_map()
ggsave(filename="SA_rivers_updated.png", width=7, height=8.5, dpi = 600, device='png', p1)
